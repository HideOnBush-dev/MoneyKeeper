...
def handle_disconnect():
    if current_user.is_authenticated:
        logger.info(f"Client disconnected: {request.sid}, User: {current_user.id}")
...
def handle_message(data):
    ...
    try:
        ...
        db.session.commit()  # Commit session first
        ...
        logger.info(f"User message saved and echoed for user {current_user.id}")

        try:
            if current_app.ai_chat is None:
                error_msg = "Tính năng AI chưa được kích hoạt. Vui lòng cấu hình GOOGLE_API_KEY."
                logger.warning(f"AI chat not available for user {current_user.id}")
                emit("error", {"data": error_msg}, room=str(current_user.id))
                return
        ...
        logger.exception(f"Critical error in message handler: {e}")
        emit("error", {"data": "Lỗi hệ thống. Vui lòng thử lại sau."}, room=str(current_user.id))
    finally:
        try:
